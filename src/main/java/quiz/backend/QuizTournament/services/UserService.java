...
import quiz.backend.QuizTournament.db.UserRepository;
...
public class UserService implements UserDetailsService {
    ...
    public boolean resetPasswordWithUsername(String username, String token, String newPassword) {
        ...
        if (userOptional.isPresent()) {
            User user = userOptional.get();
            if (user.getPassword() != null && user.getPassword().equals(token)) {
                user.setPassword(passwordEncoder.encode(newPassword)); // Hash the password
                user.setPassword(null); // Clear the reset token after successful reset
                userRepository.save(user);
                return true;
            }
        }
